		...
		*/
		public function calculate_shipping($package = array())
		{
			...
			//$verifyValue = json_encode($data);
			 $handling_fee = 0;
			 $cost = isset($data->object->deliveryPrice) ? (wc_format_decimal($data->object->deliveryPrice)) : wc_format_decimal($handling_fee);
			if(isset($data->object->preShipmentMobile->preShipmentItems)){
				$total_no_of_tasks = ($data->object->preShipmentMobile->preShipmentItems !== null) ? count($data->object->preShipmentMobile->preShipmentItems) : 0;
			}else{
				$total_no_of_tasks = 0;
			}
			$this->add_rate(array(
			...
			'total_service_charge' => isset($data->object->vat) ? $data->object->vat : 0,
			)
			));
		}
	}
