	...
	class WC_Gigl_Delivery_API
	{
		...
		public function __construct($settings = array())
		{
			...
			if ($this->env == 'Live') {
				...
				$this->request_url = 'https://dev-agilitythirdpartyapi.theagilitysystems.com/api/thirdparty/';//'https://giglthirdpartyapitestenv.azurewebsites.net/api/thirdparty/'; //'http://test.giglogisticsse.com/api/thirdparty/';
		...
		*/
		public function vendor_login($username, $password)
		{
			...
			if (empty($login_credentials)  ) {
				...
				$params = array(
               'username'        => 'testaccount@yahoo.com',//$username,
               'password'     => 'loving',//$password,
               'sessionObj'    => "",
				);
		...
		public function get_order_details($waybill)
		{
			...
			return $this->api_request('TrackAllMobileShipment/'.$waybill, $params, 'get', $access_token);
		}
		...
		public function get_lat_lng($address)
		{
			if (!empty($address)) {
			   ...
			   $params = array();
			   $geocodeResponse = $this->api_request($address, $params,'GET',$access_token);
