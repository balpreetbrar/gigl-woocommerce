	...
	class WC_Gigl_Delivery
	{
		...
		*/
		...
		public function create_order_shipping_task($order_id)
		{
			...
			$get_waybilling = get_post_meta($order_id,'gigl_delivery_waybill',true);
			...
			if (strpos($shipping_method->get_method_id(), 'gigl_delivery') !== false) {
				...
				if (isset($res->object->waybill)) {
					print_r($res->object->waybill);
					if($this->settings['mode'] == 'test'){
						$endpoint = 'https://dev-agilitythirdpartyapi.theagilitysystems.com/api/thirdparty/';
					}else{
						$endpoint = 'https://prod-agilitythirdpartyapi.theagilitysystems.com/api/thirdParty/';
					}
					...
					$this->currentWaybill = $res->object->waybill;
					 ...
					 update_post_meta($order_id, 'gigl_delivery_check_status_url', $endpoint.$res->object->waybill);
					...
					update_post_meta($order_id, 'gigl_delivery_pickup_tracking_url', $endpoint.$res->object->waybill);
					...
					update_post_meta($order_id, 'gigl_delivery_delivery_tracking_url', $endpoint.$res->object->waybill);
		...
		*/
		public function update_order_shipping_status($order_id)
		{
			...
			if ($gigl_waybill) {
				...
				if (isset($res['object']['mobileShipmentTrackings'][0]['mobileShipmentTrackingId'])) {
					$job_delivery_status = $this->statuses[$res['object']['mobileShipmentTrackings'][0]['status']];
					$tracking_id = $this->statuses[$res['object']['mobileShipmentTrackings'][0]['mobileShipmentTrackingId']];
		...
		*/
		public function add_view_order_tracking($order)
		{
			...
			if(!empty($this->currentWaybill)){
				...
				$gigl_waybill = get_post_meta($current_order_id,'gigl_delivery_waybill', true);
			}
